...
public class ParisGeneralGUITest_in_French {
	...
	public void testParisGUI_SingleChemicalMixture_in_French() {
		...
		final DecimalFormat pf = (DecimalFormat)NumberFormat.getInstance(java.util.Locale.getDefault());
		pf.applyPattern("#0.0#");
		...
		try {
			...
			Assert.assertTrue("List name is not the same as Mixture name", nameList.getItem(0).equals(table.getItem(0).getText(0)));
			
			String content = table.getItem(0).getText(1);
			Assert.assertTrue("Mixture Wt% should be "+pf.format(100.0)+", not "+content, content.equals(pf.format(100.0)));
			...
			pf.applyPattern("#0.0#");
			Assert.assertEquals("Screen2 textArray[0][0] is wrong", pf.format(11.0), text1.getText());
