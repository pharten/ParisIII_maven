...
public class Screen3 extends Screen {
	...
	public Screen3(Composite parent, int style) throws Exception {
		super(parent, style);
		...
		replacementList.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (index>=0) {
					...
					lb = ef.parse(textArray[0][1].getText()).doubleValue();
					ub = ef.parse(textArray[0][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[2][1].getText()).doubleValue();
					ub = ef.parse(textArray[2][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[7][1].getText()).doubleValue();
					ub = ef.parse(textArray[7][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[8][1].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[9][1].getText()).doubleValue();
...
//				updateReplacements = true;
				tolerances[i] = pf.parse(textArray[i][0].getText()).doubleValue();
			}
		}
	...
	private void saveProperties(State activeState) throws ParseException {
		double[] desiredVals = activeState.getADesiredVals();
		if (desiredVals == null) return;
		for (int i=0; i<10; i++) {
			if (!textArray[i][2].getText().equals(initProp[i])) {
//				updateReplacements = true;
				desiredVals[i] = ef.parse(textArray[i][2].getText()).doubleValue();
			}
		}
	}
