...
public class Screen2 extends Screen {
	...
	public Screen2(Composite parent, int style) throws Exception {
		super(parent, style);
		...
		scale.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
        		...
        		double oldScale = pf.parse(scale.getToolTipText()).doubleValue();
        		double dscale = 4.0*(sel-min)/(max-min);
        		scale.setToolTipText(pf.format(dscale));
        		
        		for (int i=0; i<7; i++) {
        			double tol = pf.parse(textArray[i][0].getText()).doubleValue();
		...
		replacementList.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (index>=0) {
					...
					value = df.parse(df.format(value)).doubleValue();
					lb = df.parse(textArray[0][1].getText()).doubleValue();
					ub = df.parse(textArray[0][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[1][1].getText()).doubleValue();
					ub = ef.parse(textArray[1][3].getText()).doubleValue();
					...
					value = df.parse(df.format(value)).doubleValue();
					lb = df.parse(textArray[2][1].getText()).doubleValue();
					ub = df.parse(textArray[2][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[3][1].getText()).doubleValue();
					ub = ef.parse(textArray[3][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[4][1].getText()).doubleValue();
					ub = ef.parse(textArray[4][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[5][1].getText()).doubleValue();
					ub = ef.parse(textArray[5][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					lb = ef.parse(textArray[6][1].getText()).doubleValue();
					ub = ef.parse(textArray[6][3].getText()).doubleValue();
					...
					value = ef.parse(ef.format(value)).doubleValue();
					ub = ef.parse(textArray[9][3].getText()).doubleValue();
...
//		System.out.println("at Screen"+screenNum);
	...
	public void updateShared() throws Exception {
		...
		if (updateReplacements) {  // some fields were changed
			saveTolerances(states.getActiveState());
			saveProperties(states.getActiveState());
			if (replacements!=null) replacements.bubbleSort();
		}
	...
	public void restore() throws Exception {
		...
		for (int i=0; i<initTol.length-1; i++) {
			initTol[i] = textArray[i][0].getText();
			initProp[i] = textArray[i][2].getText();
		}
		initProp[7] = textArray[7][1].getText();  // flash point is lower bound
	...
	private void saveTolerances(State activeState) throws ParseException {
...
//					updateReplacements = true;
					double temp = df.parse(textArray[2][2].getText()).doubleValue();
					double tol = df.parse(textArray[2][0].getText()).doubleValue();
...
//					updateReplacements = true;
					tolSI[i] = pf.parse(textArray[i][0].getText()).doubleValue();
				}
			}
	...
	private void saveProperties(State activeState) throws Exception {
...
//			updateReplacements = true;
			desiredVals[0] = Units.massConvertFrom(df.parse(textArray[0][2].getText()).doubleValue(), unit);
		}
		if (!textArray[1][2].getText().equals(initProp[1])) {
//			updateReplacements = true;
			desiredVals[1] = Units.densityConvertFrom(ef.parse(textArray[1][2].getText()).doubleValue(), unit);
		}
		if (!textArray[2][2].getText().equals(initProp[2])) {
//			updateReplacements = true;
			desiredVals[2] = Units.tempConvertFrom(df.parse(textArray[2][2].getText()).doubleValue(), unit);
		}
		if (!textArray[3][2].getText().equals(initProp[3])) {
//			updateReplacements = true;
			desiredVals[3] = Units.pressureConvertFrom(ef.parse(textArray[3][2].getText()).doubleValue(), unit);
		}
		if (!textArray[4][2].getText().equals(initProp[4])) {
//			updateReplacements = true;
			desiredVals[4] = Units.surfaceTensionConvertFrom(ef.parse(textArray[4][2].getText()).doubleValue(), unit);
		}
		if (!textArray[5][2].getText().equals(initProp[5])) {
//			updateReplacements = true;
			desiredVals[5] = Units.viscosityConvertFrom(ef.parse(textArray[5][2].getText()).doubleValue(), unit);
		}
		if (!textArray[6][2].getText().equals(initProp[6])) {
//			updateReplacements = true;
			desiredVals[6] = Units.thermalConductivityConvertFrom(ef.parse(textArray[6][2].getText()).doubleValue(), unit);
		}
		if (!textArray[7][1].getText().equals(initProp[7])) {
//			updateReplacements = true;
			desiredVals[7] = Units.tempConvertFrom(ef.parse(textArray[7][1].getText()).doubleValue(), unit);
		}
	}
	...
	private double getLb(double value, double tolerance) {
		double lb = (1.0-0.01*tolerance) * value;
		if (lb<0) lb = 0.0;
		return lb;
	}
	
	private double getUb(double value, double tolerance) {		
		return (1.0+0.01*tolerance) * value;
	}
...
//		}
	...
	private void removeFocusListeners() {
		...
		FocusListenerAdapter listener;
		for (int i=0; i<7; i++) {
			listener = (FocusListenerAdapter)focusListener[i];
			for (int j=0; j<4; j++) {
				textArray[i][j].removeFocusListener(listener);
				textArray[i][j].setEditable(false);
			}
			if (listener.isTextChanged()) updateReplacements = true;
		}
		listener = (FocusListenerAdapter) focusListener[7];
