...
//		if (this.getHtoxDermal()!=other.getHtoxDermal()) return false;
	...
	public double calculateMixtureScore(State activeState) throws Exception {
		...
		DecimalFormat ef = (DecimalFormat)NumberFormat.getNumberInstance();
...
//		Properties.calculate(this, UNIFACforPARIS.getInstance(), activeState.tempConvertToSI());
		...
		if (getFlashPoint()==-9999) return this.mixtureScore;
		
		double score = numberOfDeviationsWithOffset(desired[0], this.getMolecularWeight(), tolerance[0], df);
		score += numberOfDeviationsWithOffset(desired[1], this.getDensity(), tolerance[1], ef);
		score += numberOfDeviationsWithOffset(desired[2], this.getBoilingPoint(), tolerance[2], df);
		score += numberOfDeviationsWithOffset(desired[3], this.getVaporPressure(), tolerance[3], ef);
		score += numberOfDeviationsWithOffset(desired[4], this.getSurfaceTension(), tolerance[4], ef);
		score += numberOfDeviationsWithOffset(desired[5], this.getViscosity(), tolerance[5], ef);
		score += numberOfDeviationsWithOffset(desired[6], this.getThermalConductivity(), tolerance[6], ef);
		score += flashPointScore(desired[7], this.getFlashPoint(), 10.0, df);
		
		tolerance = activeState.getATolerances();
		desired = activeState.getADesiredVals();
		
		score += numberOfDeviationsWithOffset(desired[0], this.getInfDilActCoef_ethanol(), tolerance[0], ef);
		score += numberOfDeviationsWithOffset(desired[1], this.getInfDilActCoef_diethyl_ether(), tolerance[1], ef);
		score += numberOfDeviationsWithOffset(desired[2], this.getInfDilActCoef_acetone(), tolerance[2], ef);
		score += numberOfDeviationsWithOffset(desired[3], this.getInfDilActCoef_water(), tolerance[3], ef);
		score += numberOfDeviationsWithOffset(desired[4], this.getInfDilActCoef_benzene(), tolerance[4], ef);
		score += numberOfDeviationsWithOffset(desired[5], this.getInfDilActCoef_cis_2_heptene(), tolerance[5], ef);
		score += numberOfDeviationsWithOffset(desired[6], this.getInfDilActCoef_n_propyl_chloride(), tolerance[6], ef);
		score += numberOfDeviationsWithOffset(desired[7], this.getInfDilActCoef_n_heptadecane(), tolerance[7], ef);
		score += numberOfDeviationsWithOffset(desired[8], this.getInfDilActCoef_n_propylamine(), tolerance[8], ef);
		score += numberOfDeviationsWithOffset(desired[9], this.getInfDilActCoef_dimethyl_disulfide(), tolerance[9], ef);
	...
	private double numberOfDeviationsWithOffset(double desired, double actual, double tol, DecimalFormat df) throws ParseException {

		// format and parse steps are put in to account for differences between value and screen display
		actual = df.parse(df.format(actual)).doubleValue();
		double upperBound = df.parse(df.format(desired*(1.0+0.01*tol))).doubleValue();
		double lowerBound = df.parse(df.format(desired*(1.0-0.01*tol))).doubleValue();	
	...
	private double flashPointScore(double desired, double actual, double tol, DecimalFormat df) throws ParseException {
		...
		double offset = 0.0;
		if (actual < desired) offset += 1.0e9; // special case for flash point
...
//		if (Double.isNaN(getFlashPoint())) throw new Exception("error: FP, NaN");
...
//					System.out.println("Missing antoine constants and critical constants- can't calculate vapor pressure");
				...
				setFlashPoint(calculateMixtureFP(molFractions));
	...
	/**
	 * Calculate FlashPoint using mole fractions
	 * @param volFractions
	 * @return
	 */
	private double calculateMixtureFP(Vector<Double> molFractions) {
		double fp = calculateMixtureFP_Liaw3(molFractions);
		
		//If Liaw fails, try using Catoire method: (sometimes there is no solution)
		if (fp==-9999) {
			fp = calculateMixtureFP_Catoire(molFractions, getBoilingPoint());
			if (Double.isNaN(fp)) { //there is no solution, give up
//				System.out.println("Fail to converge, FP2");
				fp = -9999.0;
			}
		}
		return fp;
...
//		System.out.println(x1a+", "+x2a+", "+x3a+", "+x1b+", "+x2b+", "+x3b);
	...
	private double calculateMixtureBP(Vector<Double> xmol, double Pressure_kPa) {
		...
		while (frac>1e-8) {
			...
			BPmixK=BPmixKinit; //reset to initial guess
...
//				System.out.println(BPmixK);
			...
			frac = 0.5*frac;//make step smaller
//          System.out.println(frac);

		}
//		System.out.println("Fail to converge, BP");
		return -9999;
	}
...
//		System.out.println(g[0]+"\t"+g[1]);
	...
	private double calculateMixtureFP_Catoire(Vector<Double>molFractions, double BPmix_K) {
		...
		double [] ymol=calculateVaporPhaseCompositionAtNormalBP(molFractions, BPmix_K);
...
//		System.out.println("Fail to converge, FP");
		return -9999;
