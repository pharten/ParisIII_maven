...
public class Screen1 extends Screen {
	 ...
	 * @throws Exception 
	 */
	public Screen1(Composite parent, int style) throws Exception {
		super(parent, style);
...
//		System.out.println("at Screen"+screenNum);
	...
	private void loadFixedValues(State activeState) {
		...
		if (solvents.size()>0) {
			...
			chemicalWght[0].setText(pf.format(Math.round(wghts.get(0)*10000.0)*0.01));
		...
		if (solvents.size()>1) {
			...
			chemicalWght[1].setText(pf.format(Math.round(wghts.get(1)*10000.0)*0.01));
		...
		if (solvents.size()>2) {
			...
			chemicalWght[2].setText(pf.format(Math.round(wghts.get(2)*10000.0)*0.01));
		...
		if (solvents.size()>3) {
			...
			chemicalWght[3].setText(pf.format(Math.round(wghts.get(3)*10000.0)*0.01));
...
//		}
		...
		text_61.setText(pf.format(100.0));
