...
public class Screen0 extends Screen {
	...
	private double[] tempvals = {10.0, 15.0, 20.0, 25.0, 30.0, 35.0};
	private double[] presvals = {1.0};
	...
	public Screen0(Composite parent, int style) throws Exception {
		...
		String[] temperatures = new String[tempvals.length];
		for (int i=0; i<tempvals.length; i++) {
			temperatures[i] = pf.format(tempvals[i]);
		}
		
		String[] pressures = new String[presvals.length];
		for (int i=0; i<presvals.length; i++) {
			pressures[i] = pf.format(presvals[i]);
		}
...
//	        	System.out.println("SystemUnit widgetSelected");
		...
		combo_3.addSelectionListener(new SelectionListener() {
	    	...
	    	public void widgetSelected(SelectionEvent event) {
	        	...
	        	try {
					state.setSystemTemp(Double.toString(pf.parse(combo_3.getText()).doubleValue()));
...
//	        	System.out.println("SystemTemp widgetSelected");
		...
		combo_4.addSelectionListener(new SelectionListener() {
	    	...
	    	public void widgetSelected(SelectionEvent event) {
	        	State state = states.getActiveState();
	        	try {
					state.setSystemPres(Double.toString(pf.parse(combo_4.getText()).doubleValue()));
				} catch (ParseException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
...
//						DecimalFormat pc = (DecimalFormat)NumberFormat.getInstance();
//						pc.applyPattern("#0.0#");
						tableItem.setText(1,pf.format(100.0));
...
//					saveTable(states.getActiveState());
		...
		btn_2.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (table.getItemCount()==1) {
					table.getItem(0).setText(1,pf.format(100.0));
					mixture.getWghtFractions().set(0, 1.0);
				} else {
		...
		btn_3.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				carefullyDisposeReferenceWindow();
			...
			public void widgetSelected(SelectionEvent e) {
				...
				newEditor.addModifyListener(new ModifyListener() {
					...
					public void modifyText(ModifyEvent me) {
						...
						try {
							double value = pf.parse(text.getText()).doubleValue();
							tableEditor.getItem().setText(1,pf.format(value));
...
//		System.out.println("at Screen"+screenNum);
	...
	public void changeState(State state) throws Exception {
		...
		combo_3.select(combo_3.indexOf(pf.format(Double.parseDouble(state.getSystemTemp()))));
		combo_4.select(combo_4.indexOf(pf.format(Double.parseDouble(state.getSystemPres()))));
	...
	private void carefullyDisposeReferenceWindow() {
		if (reference!=null) {
			if (reference.shell!=null) {
				if (!reference.shell.isDisposed()) {
					reference.shell.dispose();
				}
			}
		}
	}
	...
	public boolean testTable() throws Exception {
		...
		for (int i=0; i<tableItem.length; i++) {
			if (tableItem[i].getText(1)=="" || pf.parse(tableItem[i].getText(1)).doubleValue()<=0.0) {
				MessageBox dialog = new MessageBox(table.getShell(), SWT.ICON_ERROR | SWT.CANCEL);
				dialog.setText("Error");
				...
				sum += pf.parse(tableItem[i].getText(1)).doubleValue();
			}
		}
		...
		if (mixture==null) {
			...
			if (table.getColumn(1).getText().equals("Wt%")) {
				...
				for (int i=0; i<chemicals.size(); i++) {
					...
					tableItem.setText(1, pf.format(Math.round(massFractions.get(i)*10000.0)*0.01));
				}
				...
				for (int i=0; i<chemicals.size(); i++) {
					TableItem tableItem = new TableItem(table, SWT.NONE);
					tableItem.setText(0, chemicals.get(i).getName());
					tableItem.setText(1, pf.format(Math.round(molFractions.get(i)*10000.0)*0.01));
				}
			}
		}
	}
	
	public void saveTable(State state) throws ParseException {
		TableItem[] tableItem = table.getItems();
		...
		if (table.getColumn(1).getText().equals("Wt%")) {
			...
			wghtFractions.clear();
			for (int i=0; i<tableItem.length; i++) {
				wghtFractions.add(pf.parse(tableItem[i].getText(1)).doubleValue()*0.01);
			}
		} else { // given in Mol%
			Vector<Double> molFractions = new Vector<Double>(tableItem.length);
			for (int i=0; i<tableItem.length; i++) {
				molFractions.add(pf.parse(tableItem[i].getText(1)).doubleValue()*0.01);
			}
	...
	public Boolean isMiscible() throws Exception {
		...
		if (table.getColumn(1).getText().equals("Wt%")) {
			...
			xmol.clear();
			for (int i=0; i<tableItem.length; i++) {
				xmol.add(pf.parse(tableItem[i].getText(1)).doubleValue()*0.01);
			}
		}
